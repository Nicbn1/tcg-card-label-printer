# NIIMBOT D11 Android Verification
...
3. Pair the NIIMBOT D11 in **Android Settings → Connected devices → Pair new device** before opening PriceTag.
...
5. Confirm the D11 prints the selected preset with the expected name, price, condition, Figureheadz website, generated date, and stale-price notice when applicable.
6. Repeat from the batch queue and a history reprint.
7. Turn off the D11 and retry. PriceTag must retain the saved history/queue item and show a connection error rather than reporting a successful print.
