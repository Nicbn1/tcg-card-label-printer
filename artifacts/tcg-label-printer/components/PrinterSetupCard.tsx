...
export function PrinterSetupCard() {
  ...
  const refreshPairedDevices = async () => {
    ...
    try {
      ...
      if (!allowed.granted) {
        setMessage('Allow Nearby devices access to list your paired N12.');
        return;
      }
      const pairedDevices = await getPairedPrinters();
      setDevices(pairedDevices);
      setMessage(
        pairedDevices.length
          ? 'Choose the N12 you paired in Android Settings.'
          : 'No paired Bluetooth devices found. Pair the N12 in Android Settings first.',
      );
    } catch (error: unknown) {
      setMessage(error instanceof Error ? error.message : 'Could not list paired Bluetooth devices.');
    } finally {
      setLoading(false);
    }
  };
      ...
      <View style={styles.header}>
        ...
        <View style={styles.copy}>
          ...
          <Text style={[styles.subtitle, { color: colors.mutedForeground, fontFamily: 'Inter_400Regular' }]}>
            {isNativeBuild ? 'Choose a paired printer for labels' : 'Native Android build required'}
          </Text>
        </View>
        ...
        <>
          <Text style={[styles.body, { color: colors.mutedForeground, fontFamily: 'Inter_400Regular' }]}>
            {savedAddress
              ...
              : 'Pair the N12 in Android settings, then choose it below.'}
          </Text>
          ...
          <View style={styles.actions}>
            <TouchableOpacity
              ...
              testID="refresh-paired-printers"
            >
              {loading ? (
