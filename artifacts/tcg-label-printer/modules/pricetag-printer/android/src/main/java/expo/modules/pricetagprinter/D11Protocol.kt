...
internal object D11Protocol {
  ...
  fun imageRowFrame(row: Int, rowBytes: ByteArray, printheadPixels: Int = 96): ByteArray {
    require(printheadPixels > 0 && printheadPixels % 8 == 0)
    require(rowBytes.size == printheadPixels / 8) {
      "D11 bitmap row does not match the configured print-head width."
    }

    /*
     * D11 firmware accepts zeroes for the three optional black-pixel counters.
     * Keeping every row in the fixed-width 0x85 form avoids firmware-dependent
     * empty/indexed-row optimizations that can be acknowledged but rendered as
     * white rows.
     */
    return buildFrame(
      CMD_PRINT_BITMAP_ROW,
      u16(row) + byteArrayOf(0x00, 0x00, 0x00, 0x01) + rowBytes,
    )
  }
